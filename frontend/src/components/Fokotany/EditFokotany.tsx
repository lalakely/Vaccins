...
function EditFokotany({ fokotany, onEditSuccess }: EditFokotanyProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    px: fokotany.px,
    py: fokotany.py,
    nombre_personne: fokotany.nombre_personne || 0
  });
  
  // Position pour la carte - conversion des coordonnées en nombres
  const initialPosition: [number, number] = [Number(fokotany.py), Number(fokotany.px)];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await fetch(buildApiUrl(`/api/fokotany/${fokotanyId}`), {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        showSuccess('Fokotany mis à jour', 'Les informations du fokotany ont été modifiées avec succès.');
        onEditSuccess();
        setIsEditing(false);
      } else {
        const errorData = await response.json();
        showError('Erreur de mise à jour', errorData.message || 'Impossible de mettre à jour le fokotany.');
      }
    } catch (error) {
      console.error('Erreur lors de la mise à jour:', error);
      showError('Erreur de mise à jour', 'Une erreur s\'est produite lors de la mise à jour du fokotany.');
    } finally {
      setLoading(false);
    }
  };
